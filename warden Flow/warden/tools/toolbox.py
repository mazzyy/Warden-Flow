...
class ToolBox:
    def __init__(
        ...
        *,
        ...
        # The local directory the DELIVER workflow reads to containerize a
        # service. None for the incident (OPERATE) workflow, which reads the
        # cluster and the GitOps repo instead.
    ...
    def build(self, tool_names: list[str], *, max_changed_lines: int = 0) -> list[Callable]:
        ...
        factories = {
            ...
            "list_source_files": self._list_source_files,
            "read_source_file": self._read_source_file,
    ...
    # -- local source reads: the DELIVER workflow's input ------------------

    _SKIP_PARTS = frozenset({"__pycache__", ".venv", ".git", "node_modules", ".ruff_cache"})

    def _list_source_files(self) -> Callable:
        root = self._source_root

        async def list_source_files(subdir: str = "") -> dict:
            """List the files in the local source directory being delivered.

            Call this before read_source_file to find the entrypoint and the
            dependency manifest instead of guessing their names.

            Args:
                subdir: Optional subdirectory to limit the listing to.
            """
            if root is None:
                return {"error": "no source directory configured"}
            base = (root / subdir).resolve()
            if root not in base.parents and base != root:
                return {"error": "path escapes the source root"}
            files = [
                str(p.relative_to(root))
                for p in sorted(base.rglob("*"))
                if p.is_file() and ToolBox._SKIP_PARTS.isdisjoint(p.parts)
            ]
            return {"files": files[:200], "root": root.name}

        return list_source_files

    def _read_source_file(self) -> Callable:
        root = self._source_root

        async def read_source_file(path: str) -> dict:
            """Read a file from the local source directory being delivered.

            Args:
                path: Source-relative path, for example "main.py" or
                    "requirements.txt".
            """
            if root is None:
                return {"error": "no source directory configured"}
            ...
            if root not in target.parents:
                return {"error": "path escapes the source root", "path": path}
            if not target.is_file():
                return {"error": "not found", "path": path}
            try:
                return {"path": path, "content": target.read_text()[:20000]}
