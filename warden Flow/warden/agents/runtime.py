...
async def _execute(
    *,
    ...
    # Record the model that will actually answer, not just the manifest default:
    # an override (a scripted model offline, or Azure/OpenAI/Anthropic via
    # LiteLlm) is what really ran, so that is what the trace and audit should show.
    if model_override is None:
        model_name = manifest.spec.model
    elif isinstance(model_override, str):
        model_name = model_override
    else:
        model_name = getattr(model_override, "model", manifest.spec.model)
